...
// GET /api/apps/developer?devId=xxx&platform=android
router.get('/developer', async (req, res) => {
  const { devId, platform = 'android', country = 'us', lang = 'en' } = req.query;
  if (!devId) return res.status(400).json({ error: 'devId is required' });

  const cacheKey = `developer:${devId}:${platform}:${country}`;
  const cached = getCached(cacheKey);
  if (cached) return res.json(cached);
  ...
  try {
    let apps = [];
    if (platform === 'android') {
      apps = await gplay.developer({ devId, country, lang });
    } else {
      apps = await store.developer({ id: devId, country });
    }
    setCache(cacheKey, apps, 3600);
    res.json(apps);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
// GET /api/apps/top?category=HEALTH_AND_FITNESS&collection=TOP_FREE&country=us&num=30
router.get('/top', async (req, res) => {
  const { category, collection = 'TOP_FREE', country = 'us', lang = 'en', num = 30 } = req.query;

  const cacheKey = `top:${category}:${collection}:${country}:${num}`;
  const cached = getCached(cacheKey);
  if (cached) return res.json(cached);
  ...
  try {
    const opts = {
      ...
      num: parseInt(num), country, lang, fullDetail: false
    };
    ...
    const result = apps.map(a => ({
      ...
      installs: a.installs, free: a.free, genre: a.genre
    }));
